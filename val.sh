...
for user in "${!USERS[@]}"; do
    ...
    result=$(sshpass -p "$password" ssh -p "$SSH_PORT" \
        -o "StrictHostKeyChecking=no" \
        -o "UserKnownHostsFile=/dev/null" \
        -o "ConnectTimeout=5" \
        "$user@$SSH_HOST" "whoami" 2>&1)
...
echo ""
echo -e "SSH Summary: ${GREEN}$SSH_SUCCESS${NC} successful, ${RED}$SSH_FAIL${NC} failed"
echo ""
...
result=$(sshpass -p "password123" ssh -p "$SSH_PORT" \
    -o "StrictHostKeyChecking=no" \
    -o "UserKnownHostsFile=/dev/null" \
    "player1@$SSH_HOST" "whoami" 2>&1)
...
result=$(sshpass -p "password123" ssh -p "$SSH_PORT" \
    -o "StrictHostKeyChecking=no" \
    -o "UserKnownHostsFile=/dev/null" \
    "player1@$SSH_HOST" "su - backup -c whoami" 2>&1)
